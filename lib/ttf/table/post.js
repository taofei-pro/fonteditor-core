...
var _default = _table["default"].create('post', [], {
  read: function read(reader, ttf) {
    var format = reader.readFixed(this.offset); // 读取表头

    var tbl = new Posthead(this.offset).read(reader, ttf); // format2
  ...
  size: function size(ttf) {
    ...
    ttf.post.maxMemType1 = numberOfGlyphs; // version 3 不设置post信息
    ...
    for (var i = 0; i < numberOfGlyphs; i++) {
      ...
      if (i === 0) {
        ...
        var glyf = ttf.glyf[i];
        var unicode = glyf.unicode ? glyf.unicode[0] : undefined;
        var unicodeNameIndex = unicode !== undefined ? _unicodeName["default"][unicode] : undefined;
