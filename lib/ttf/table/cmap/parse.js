...
function readSubTable(reader, ttf, subTable, cmapOffset) {
  ...
  var glyphCount;
  subTable.format = reader.readUint16(startOffset); // 0～256 紧凑排列

  if (subTable.format === 0) {
    var format0 = subTable; // 跳过format字段
    ...
    var format2 = subTable; // 跳过format字段
    ...
    var format4 = subTable; // 跳过format字段
    ...
    var segCount = format4.segCountX2 / 2; // end code
    ...
    format4.reservedPad = reader.readUint16(); // start code
    ...
    format4.startCode = startCode; // idDelta
    ...
    format4.idRangeOffsetOffset = reader.offset; // idRangeOffset
    ...
    format4.idRangeOffset = idRangeOffset; // 总长度 - glyphIdArray起始偏移/2

    glyphCount = (format4.length - (reader.offset - startOffset)) / 2; // 记录array offset

    format4.glyphIdArrayOffset = reader.offset; // glyphIdArray
    ...
    format6.entryCount = reader.readUint16(); // 记录array offset

    format6.glyphIdArrayOffset = reader.offset;
    var glyphIndexArray = [];
    var entryCount = format6.entryCount; // 读取字符分组
    ...
    var groups = [];
    var nGroups = format12.nGroups; // 读取字符分组
...
function parse(reader, ttf) {
  var tcmap = {}; // eslint-disable-next-line no-invalid-this
  ...
  var offset = reader.offset; // 使用offset读取，以便于查找
