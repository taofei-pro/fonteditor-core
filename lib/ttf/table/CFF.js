...
var _default = _table["default"].create('cff', [], {
  read: function read(reader, font) {
    ...
    cff.gsubrsBias = calcCFFSubroutineBias(globalSubrIndex.objects); // 顶级字典数据
    ...
    cff.privateDict = privateDict; // 解析glyf数据和名字
    ...
    cff.glyf = []; // only parse subset glyphs
    ...
    if (subset && subset.length > 0) {
      ...
      var codes = font.cmap; // unicode to index
