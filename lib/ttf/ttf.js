...
function merge(ttf, imported) {
  ...
  var list = imported.glyf.filter(function (g) {
    return (// 简单轮廓
      g.contours && g.contours.length // 非预定义字形
  ...
  if (options.adjustGlyf) {
    ...
    var scale = 1; // 调整glyf对导入的轮廓进行缩放处理
  ...
  _createClass(TTF, [{
    ...
    value: function sortGlyf() {
      ...
      if (glyf.length > 1) {
        ...
        var notdef = glyf.shift(); // 按代码点排序, 首先将空字形排到最后，然后按照unicode第一个编码进行排序
    ...
    value: function setOS2(fields) {
      (0, _lang.overwrite)(this.ttf['OS/2'], fields, ['usWinAscent', 'usWinDescent', 'sTypoAscender', 'sTypoDescender', 'sTypoLineGap', 'sxHeight', 'bXHeight', 'usWeightClass', 'usWidthClass', 'yStrikeoutPosition', 'yStrikeoutSize', 'achVendID', // panose
