...
function ttf2eot(ttfBuffer) {
  ...
  var eot = {};
  eot.head = eotHead.read(new _reader["default"](new ArrayBuffer(eotHeaderSize))); // set fields
  ...
  eot.head.Padding1 = 0;
  var ttfReader = new _reader["default"](ttfBuffer); // 读取ttf表个数
  ...
  ttfReader.seek(12); // 需要读取3个表内容，设置3个byte
  ...
  eot.head.EOTSize = eotHeaderSize + 4 + eot.FamilyNameSize + 4 + eot.StyleNameSize + 4 + eot.VersionNameSize + 4 + eot.FullNameSize + 2 + eot.head.FontDataSize; // 这里用小尾方式写入

  var eotWriter = new _writer["default"](new ArrayBuffer(eot.head.EOTSize), 0, eot.head.EOTSize, true); // write head

  eotHead.write(eotWriter, eot); // write names
  ...
  eotWriter.writeUint16(0); // write rootstring
