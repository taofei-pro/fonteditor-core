...
function parseFont(xmlDoc, ttf) {
  ...
  if (fontFaceNode) {
    ...
    ttf.head.unitsPerEm = +(fontFaceNode.getAttribute('units-per-em') || 0); // 解析panose, eg: 2 0 6 3 0 0 0 0 0 0
    ...
    OS2.bXHeight = +(fontFaceNode.getAttribute('x-height') || 0); // 解析bounding
    ...
    ttf.post.underlinePosition = +(fontFaceNode.getAttribute('underline-position') || 0); // unicode range
...
function parseGlyf(xmlDoc, ttf) {
  var missingNode = xmlDoc.getElementsByTagName('missing-glyph')[0]; // 解析glyf
...
function parseXML(xmlDoc, options) {
  ...
  var ttf; // 如果是svg字体格式，则解析glyf，否则解析path
